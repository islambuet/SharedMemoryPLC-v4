...
class Tag extends EventEmitter {
    ...
    parseReadMessageResponse(data) {
        ...
        if (this.state.tag.bitIndex !== null) 
        {
            ...
            //Moe
            //console.log('This function is parseReadMessageResponse and data type is STRUCT. Data Length: ', data.length);
            //Moe
    ...
    parseReadMessageResponseValueForBitIndex(data) {
        const { tag } = this.state;
        const { SINT, INT, UINT, DINT, UDINT, BIT_STRING } = Types;
        //Moe
        //console.log('This function is parseReadMessageResponseValueForBitIndex and data length: ', data.length);
        //Moe
        // Read Tag Value
        ...
        switch (this.state.tag.type) {
            ...
            case UDINT:
                this.controller_value =
                    (data.readUInt32LE(2) & (1 << tag.bitIndex)) == 0 ? false : true;
                break;
            //***********************End Added by AClusterLLC
            default:
                throw new Error(
                    "Data Type other than SINT, INT, UINT, DINT, UDINT or BIT_STRING returned when a Bit Index was requested"
                );
        }
    ...
    parseReadMessageResponseValueForAtomic(data) {
        const { SINT,INT, UINT, DINT, UDINT, REAL, BOOL, LINT, BIT_STRING } = Types;
        ...
        switch (this.state.tag.type) {
            ...
            case UINT:
               ...
               // throw new Error(
                //    `I got UINT data and do not know what to do!!: ${this.state.tag.type}`
                //);
                break;
            //*********************** End Added by AClusterLLC
            case INT:
    ...
    generateWriteMessageRequestForBitIndex(value) {
        ...
        switch (tag.type) {
            ...
            case UDINT:
                buf = Buffer.alloc(10);
                buf.writeInt16LE(4); //mask length**************Moe This may need to be changed to buf.writeInt32LE(4);
                buf.writeUInt32LE(value ? 1 << tag.bitIndex : 0, 2); // or mask
                buf.writeUInt32LE(value ? -1 : -1 & ~(1 << tag.bitIndex), 6); // and mask
                break;
            //***********************End Added by AClusterLLC
            default:
                throw new Error(
    ...
    generateWriteMessageRequestForAtomic(value, size) {
        const { tag } = this.state;
        const { SINT, INT, UINT, DINT, UDINT, REAL, BOOL, LINT } = Types;
        // Build Message Router to Embed in UCMM
